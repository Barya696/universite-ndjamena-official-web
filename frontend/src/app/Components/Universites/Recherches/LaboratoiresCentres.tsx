  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-4 pb-12 space-y-8">
        {LABOS.map((bloc) => (
          <article key={bloc.faculte} className="rounded-lg border overflow-hidden" style={{ borderColor: "#e2e8f0" }}>
            <div className="px-5 py-4" style={{ background: bloc.color }}>
              <h2 className="font-bold text-white text-lg" style={{ fontFamily: "Georgia, serif" }}>{bloc.faculte}</h2>
            </div>
            <div className="bg-white divide-y divide-gray-100">
              {bloc.list.map((l) => (
                <div key={l.acronyme} className="grid md:grid-cols-[0.7fr_1.7fr_0.9fr_1.2fr_1.5fr] gap-3 px-5 py-4 text-sm items-start">
                  <div className="inline-flex items-center justify-start">
                    <span
                      className="inline-block px-2.5 py-1 rounded text-sm font-bold"
                      style={{ background: NAVY, color: "#fff", fontFamily: "Georgia, serif" }}
                    >
                      {l.acronyme}
                    </span>
                  </div>
                  <div className="font-semibold" style={{ color: NAVY }}>{l.nom}</div>
                  <div className="text-[#444]">{l.type}</div>
                  <div className="text-[#444]">{l.directeurs}</div>
                  <div className="text-[#555] leading-relaxed">{l.thematiques}</div>
                </div>
              ))}
            </div>
          </article>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        ...
        >
          Centres d'excellence & plateformes transversales
        </h2>
        <div className="grid md:grid-cols-2 gap-6">
          {CENTRES.map((c) => (
            <article key={c.acronyme} className="rounded-lg border overflow-hidden" style={{ borderColor: "#e2e8f0" }}>
              <div className="px-6 py-5 flex items-start gap-4" style={{ background: NAVY }}>
                <span
                  className="inline-flex items-center justify-center w-12 h-12 rounded-lg shrink-0 text-2xl"
                  style={{ background: "rgba(200,168,75,0.14)" }}
                >
                  {c.icone}
                </span>
                <div>
                  <h3 className="font-bold text-white text-base leading-snug" style={{ fontFamily: "Georgia, serif" }}>
                    {c.nom}
                  </h3>
                  <span
                    className="inline-block mt-1 text-[11px] tracking-widest uppercase"
                    style={{ color: GOLD }}
                  >
                    {c.acronyme}
                  </span>
              ...
              <div className="p-5 bg-white">
                <p className="text-sm text-[#444] leading-relaxed">{c.desc}</p>
              </div>
            </article>
          ))}
        </div>
